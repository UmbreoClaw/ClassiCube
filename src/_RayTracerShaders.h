...
static const char* const RT_TRACE_SRC =
	...
	"	if (fullBright(h.block)) return includeEmissive ? alb : vec3(0.0);\n"
	"\n"
	...
	"	/*  behind it becomes the primary hit for lighting, depth and denoising. Up to 4 layers */\n"
	"	/*  are accumulated front to back, so a water column standing in a pool shows the pool */\n"
	"	for (int layer = 0; layer < 4 && hit && h.draw == uint(DRAW_TRANSLUCENT); layer++) {\n"
	...
	"\n"
	"		if (layer == 0 && (flags & FLAG_REFLECTIONS) != 0) {\n"
	...
	"			wcol += refl * F;\n"
	"		}\n"
	"		if (layer == 0) waterT = h.t;\n"
	"		water.rgb += (1.0 - water.a) * wcol;\n"
	"		water.a   += (1.0 - water.a) * wtex.a;\n"
	"\n"
	"		hit = traceRay(h.pos + rd * 1e-3, rd, misc.y, false, false, h);\n"
	"	}\n"
	"	if (hit && h.draw == uint(DRAW_TRANSLUCENT)) hit = traceRay(h.pos + rd * 1e-3, rd, misc.y, false, true, h);\n"
	"\n"
	"	if (!hit) {\n"
	...
	"	/* With rate 2 only half the pixels trace GI each frame (alternating checkerboard); */\n"
	"	/*  the temporal pass keeps the other half's history and the blur fills in the rest */\n"
	"	bool  giSample = worldSize.w <= 1 || (((px.x + px.y + screen.z) & 1) == 0);\n"
	"	float giValid  = 1.0;\n"
	"\n"
	...
	"\n"
	"		if ((flags & FLAG_GI) != 0 && !giSample) {\n"
	"			indirect = vec3(0.0);\n"
	"			giValid  = 0.0;\n"
	"		} else if ((flags & FLAG_GI) != 0) {\n"
	...
	"	imageStore(indirectImg, px, vec4(indirect, giValid));\n"
	"	imageStore(waterImg,    px, water);\n"
	"}\n"
	"\n";
...
static const char* const RT_TEMPORAL_SRC =
	...
	"#define MAX_DIR_HISTORY 8.0\n"
	"\n"
	"/* Clamps a history colour to the range of the current frame's neighbourhood, so lighting that\n"
	"   changed (a shadow moving with a player, a light that went out) doesn't leave a trail */\n"
	"vec3 clampToNeighbourhood(sampler2D cur, ivec2 px, vec3 history, int radius, bool useValidity) {\n"
	"	vec3 mn = vec3(1e30), mx = vec3(-1e30), mean = vec3(0.0), m2 = vec3(0.0);\n"
	"	float n = 0.0;\n"
	"	for (int j = -radius; j <= radius; j++) {\n"
	"		for (int i = -radius; i <= radius; i++) {\n"
	"			ivec2 q = clamp(px + ivec2(i, j), ivec2(0), screen.xy - 1);\n"
	"			vec4 s = texelFetch(cur, q, 0);\n"
	"			if (useValidity && s.a <= 0.5) continue;\n"
	"			mn = min(mn, s.rgb); mx = max(mx, s.rgb);\n"
	"			mean += s.rgb; m2 += s.rgb * s.rgb; n += 1.0;\n"
	"		}\n"
	"	}\n"
	"	if (n < 1.0) return history;\n"
	"	if (!useValidity) return clamp(history, mn, mx);\n"
	"	/* noisy inputs: use mean +- 2 standard deviations instead of the hard min/max */\n"
	"	mean /= n;\n"
	"	vec3 sigma = sqrt(max(m2 / n - mean * mean, vec3(0.0)));\n"
	"	return clamp(history, mean - 2.0 * sigma, mean + 2.0 * sigma);\n"
	"}\n"
	"\n"
	"void main() {\n"
	"	ivec2 px = ivec2(gl_GlobalInvocationID.xy);\n"
	...
	"				history = ai.w;\n"
	"				prevDir = clampToNeighbourhood(directCur,   px, ad.rgb, 1, false);\n"
	"				prevInd = clampToNeighbourhood(indirectCur, px, ai.rgb, 2, true);\n"
	"			}\n"
	"		}\n"
	"	}\n"
	"\n"
	"	float dirHistory = min(history + 1.0, MAX_DIR_HISTORY);\n"
