...
static const char* const RT_ATROUS_SRC =
	...
	"layout(rgba32f, binding = 0) uniform readonly  image2D gbufCur;\n"
	"layout(rgba32f, binding = 1) uniform readonly  image2D normalCur;\n"
	"layout(rgba16f, binding = 2) uniform readonly  image2D srcImg;\n"
	"layout(rgba16f, binding = 3) uniform writeonly image2D dstImg;\n"
	"\n"
	...
	"\n"
	"	vec4 g = imageLoad(gbufCur, px);\n"
	"	vec4 c = imageLoad(srcImg, px);\n"
	"	if (g.w < 0.0) { imageStore(dstImg, px, c); return; }\n"
	"\n"
	"	vec4  n = imageLoad(normalCur, px);\n"
	...
	"\n"
	"			vec4 gq = imageLoad(gbufCur, q);\n"
	"			if (gq.w < 0.0) continue;\n"
	"			vec4 nq = imageLoad(normalCur, q);\n"
